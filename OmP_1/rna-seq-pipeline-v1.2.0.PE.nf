...
params.gtf = "/path/to/mm10.gtf"  // Add path to your mm10 GTF file
...
process TRIMMOMATIC {
    ...
    output:
    tuple val(sample_id), path("${sample_id}_1P.fastq.gz"), path("${sample_id}_2P.fastq.gz"), emit: trimmed_paired
    tuple val(sample_id), path("${sample_id}_1U.fastq.gz"), path("${sample_id}_2U.fastq.gz"), emit: trimmed_unpaired
...
process FASTQC_TRIMMED {
    ...
    script:
    """
    fastqc -q -t ${task.cpus} ${reads1} ${reads2}
    """
}
...
process HISAT2_ALIGN {
    ...
    script:
    """
    hisat2 -p ${task.cpus} -x ${params.genome_index} -1 ${reads1} -2 ${reads2} | \
...
workflow {
    ...
    FASTQC_TRIMMED(trimmed_reads.trimmed_paired)
    
    aligned_reads = HISAT2_ALIGN(trimmed_reads.trimmed_paired)
